...
```bash
npm install && npx expo install @react-navigation/native @react-navigation/native-stack @react-navigation/bottom-tabs react-native-screens react-native-safe-area-context expo-image-picker expo-location react-native-webview expo-linear-gradient expo-font @expo-google-fonts/inter
```
...
- **Leaflet (via WebView)**: Mapas interativos de Caxias-MA.
- **Expo Image Picker**: Acesso à câmera e galeria.
- **Expo Location**: GPS e geolocalização em tempo real.
- **Context API**: Gerenciamento de estado de usuários e reportes.
